...
class PendatangController extends Controller
{
    ...
    {
        $pendatang = PendatangModel::with(['warga.kartu_keluarga'])->get();
        ...
        $sheet->mergeCells('A5:L5')->setCellValue('A5', 'LAPORAN DATA PENDATANG WARGA');
        ...
        foreach ($pendatang as $p) {
            ...
            $sheet->setCellValue("K{$row}", \Carbon\Carbon::parse($p->tanggal_datang)->format('d-m-Y'));
            $sheet->setCellValue("L{$row}", $p->alamat_lama);
            $row++;
        }
        ...
        $filename = 'Laporan_Data_Pendatang.xlsx';
