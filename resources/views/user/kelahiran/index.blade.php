...
@section('content')
<div class="p-6">
    <h2 class="text-xl font-bold mb-4">LIHAT DATA KELAHIRAN</h2>

    <form method="GET" action="{{ route('user.kelahiran.index') }}" class="mb-4 space-y-2">
        ...
        <table class="min-w-full text-xs text-left">
            <thead>
                <tr class="bg-cyan-700 text-white text-center">
                    ...
                    <th class="border px-2 py-1">Nama Bayi</th>
                    ...
                    <th class="border px-2 py-1">NIK Ayah</th>
                    <th class="border px-2 py-1">NIK Ibu</th>
            ...
            <tbody>
                @forelse ($kelahiran as $item)
                    <tr class="border-t">
                        ...
                        <td class="border px-2 py-1">{{ $item->kartu_keluarga->no_kk ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->nama_bayi ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->jenis_kelamin ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->tempat_lahir ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->tanggal_lahir ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->nik_ayah ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->nik_ibu ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->kartu_keluarga->desa ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->kartu_keluarga->alamat ?? '-' }}</td>
                    </tr>
                @empty
                    <tr>
    ...
    <div class="flex gap-2 mt-4">
        <a href="{{ route('user.kelahiran.exportPdf', request()->query()) }}" 
        target="_blank" 
        class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm">
        <i class="fas fa-file-pdf mr-1"></i> Cetak PDF
        </a>

        <a href="{{ route('user.kelahiran.exportExcel', request()->query()) }}" 
