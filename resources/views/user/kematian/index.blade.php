...
@section('content')
<div class="p-6">
    <h2 class="text-xl font-bold mb-4">LIHAT DATA KEMATIAN</h2>

    <form method="GET" action="{{ route('user.kematian.index') }}" class="mb-4 space-y-2">
        ...
        <table class="min-w-full text-xs text-left">
            <thead>
                <tr class="bg-cyan-700 text-white text-center">
                    ...
                    <th class="border px-2 py-1">Tanggal Kematian</th>
                    <th class="border px-2 py-1">Tempat Kematian</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($kematian as $item)
                    <tr class="border-t">
                        ...
                        <td class="border px-2 py-1">{{ $item->tanggal_kematian ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $item->tempat_kematian ?? '-' }}</td>
                    </tr>
                @empty
                    <tr>
    ...
    <div class="flex gap-2 mt-4">
        <a href="{{ route('user.kematian.exportPdf', request()->query()) }}" 
        target="_blank" 
        class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm">
        <i class="fas fa-file-pdf mr-1"></i> Cetak PDF
        </a>

        <a href="{{ route('user.kematian.exportExcel', request()->query()) }}" 
