...
<div class="max-w-xl mx-auto p-4">
    ...
    <form action="{{ route('admin.perpindahan.update', $perpindahan->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="alamat_baru" value="{{ $perpindahan->alamat_baru }}" class="w-full border p-2" required>
        </div>

        <div>
            <label class="block font-medium">Tanggal Pindah</label>
            <input type="date" name="tanggal_pindah" value="{{ $perpindahan->tanggal_pindah }}" class="w-full border p-2" required>
        </div>
